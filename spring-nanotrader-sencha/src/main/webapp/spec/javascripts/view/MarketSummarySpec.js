...
describe('SpringTrader.view.MarketSummary', function() {
    ...
    it('shows indices', function() {
        ...
        expect(Ext.DomQuery.selectNode('#ms-volume', domEl).textContent).toEqual('22497.00');
    ...
    it('shows days gainers prices', function() {
        expect(Ext.DomQuery.select('.market-summary .gainers .prices', domEl).map(function(el) {return el.textContent}).join(', ')).toEqual('228.26, 241.24, 215.17');
    });
    ...
    it('shows days losers prices', function() {
        expect(Ext.DomQuery.select('.market-summary .losers .prices', domEl).map(function(el) {return el.textContent}).join(', ')).toEqual('108.99, 119.99, 94.04');
    });
